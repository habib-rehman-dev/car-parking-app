...
export async function getAllActiveSessions(req, res, next) {
  ...
  if(result == [] || result.length == 0) {
    throw new ApiError("No active parking sessions found", 404, "NO_ACTIVE_SESSIONS");
  }
...
export async function getHistory(req, res, next) {
  try {
    ...
    if(!result || result.length === 0) {
      throw new ApiError("No exited parking sessions found", 404, "NO_EXITED_SESSIONS");
    }

   res.json({ result, message: "session history fetched successfully", success: true });
  } catch (err) {
